...
/**
 * Класс для отправки посылок на другие узлы,
 * а также обновлении статуса при доставке посылки.
 */
public class ParcelSender {
    ...
    /**
     * Определяет следующее действие с посылкой.
     * Если узел последний, то вызывает функцию обратного
     * вызова на изменения статуса в предыдущих узлах пути.
     * Если узел не последний, то отправляет посылку на следующий узел.
     *
    ...
    /**
     * Отправляет посылку на следующий узел в пути.
     * Проверяет доступность узла. Если он доступен,
     * то отправляет на него POST-запрос с посылкой в теле.
     * Если узел недоступен, то через определённое время
     * отправляет повторный запрос.
     * При доступности узла меняет статус посылки 'On Next Point'.
     *
     * @param  parcelEntity  объект класса {@link ParcelEntity}.
     * @see HttpRequester
     */
    ...
    /**
     * Перегрузка метода {@link ParcelSender#updateStatusCallback(ParcelEntity)}.
     * Сделано с целью оптимизации и отсутствия избыточных
     * запросов в базу данных.
     *
     * @param  uuid  UUID посылки {@link ParcelEntity#uuid}
    ...
    /**
     * Функция обратного вызова по изменению статуса
     * посылки на 'Delivered'.
     * Если данный узел тот, с которого изначально
     * отправлялась посылка, то выполнение метода прекращается.
     * Если нет, то отправляется POST-запрос с UUID посылки
     * в качестве параетров запроса.
     *
     * @param  parcelEntity  объект класса {@link ParcelEntity}.
     * @see HttpRequester
     */
