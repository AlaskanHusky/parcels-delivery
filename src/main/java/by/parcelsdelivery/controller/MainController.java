...
/**
 * Главный контроллер, который обрабатывает запросы,
 * связанные с отправкой посылок {@link ParcelEntity}.
 */
@Controller
public class MainController {
    ...
    /**
     * Обрабатывает запрос пользователя по корневому пути.
    ...
    /**
     * Обрабатывает запрос с формы для отправки посылки.
     * Заполняет поля посылки данными с формы.
     * Сохраняет посылку в базе данных.
     * Перед сохранением, для посылки генерируется уникальный UUID.
     * После этого отправляет на следующую точку в пути посылки.
     *
    ...
    /**
     * Обрабатывает запрос от другой точки с посылкой в теле запроса
     * Устанавливает ей статус "Transit", так как посылка приходит
     * со статусом "On Next Point".
     * Если точка последняя в пути, то вызывается функция обратного
     * вызова {@link ParcelSender#updateStatusCallback(ParcelEntity)}
     * изменения статуса посылки на предыдущих точках.
     * Если точка не последняя, то посылка передаётся следующей по очереди
     * с помощью вызова функции {@link ParcelSender#sendParcel(ParcelEntity)}.
     *
     * @param   parcelEntity  объект класса {@link ParcelEntity}
     * @return                перенаправление на главную страницу
     * @see                   ParcelSender
    ...
    /**
     * Обрабатывает запрос от другой точки с UUID посылки в теле запроса.
     * Вызывает функцию {@link ParcelSender#updateStatusCallback(ParcelEntity)}
     * на изменение статуса посылки.
     *
     * @param   uuid  уникальный идентификатор посылки {@link ParcelEntity#uuid}
     * @return        перенаправление на главную страницу
     * @see           ParcelSender
     */
